...
AS $$
  -- ALL live cashflow_transactions (native + posted-from-legacy)
  SELECT
  ...
  -- Only UNPOSTED legacy CFT data (posted ones are already in live table above)
  SELECT
  ...
  WHERE lm.table_name = 'cashflow_transactions'
    ...
    AND COALESCE(lm.is_posted, false) = false
