...
-- Update outstanding calculation: Payout + Fees + Loadings Receivable - Repaid - WriteOff
...
BEGIN
  ...
  WITH loan_entries AS (
    ...
    UNION ALL
    -- BK: Fees (1975), Loadings Receivable (1984)
    SELECT 
      (notes::json->>'EntityID') AS client_acct_id,
      (notes::json->>'Type_TransactionEntryID') AS entry_type,
      COALESCE((notes::json->>'Debit')::numeric, 0) AS debit,
  ...
  aggregated AS (
    SELECT
      ...
      COALESCE(SUM(CASE WHEN entry_type = '1975' THEN credit ELSE 0 END), 0) AS fees,
...
-- Update transaction detail: show 1962, 1975, 1984 on credit; 1978, 2002 on debit
...
BEGIN
  RETURN QUERY
  -- CFT: Payout, Repayment, Write-off
  SELECT
    lm.legacy_id,
  ...
  SELECT
    ...
    COALESCE(
      ...
      CASE (lm.notes::json->>'Type_TransactionEntryID')
        ...
        WHEN '1984' THEN 'Loan FL Receivable'
  ...
  WHERE lm.table_name = 'bookkeeping'
    AND lm.tenant_id = p_tenant_id
    AND (lm.notes::json->>'EntityID') = p_legacy_entity_id
    AND (lm.notes::json->>'Type_TransactionEntryID') IN ('1975', '1984')
