...
AS $$
  ...
  -- EXCLUDE entries that have already been posted to the live cashflow_transactions table
  SELECT
  ...
  WHERE lm.table_name = 'cashflow_transactions'
    ...
    -- Exclude legacy entries already posted to live table
    AND NOT EXISTS (
      SELECT 1 FROM public.cashflow_transactions ct2
      WHERE ct2.legacy_transaction_id = lm.legacy_id
