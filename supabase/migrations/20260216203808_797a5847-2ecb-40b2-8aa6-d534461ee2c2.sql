...
-- Update loan transactions schedule to match legacy report: BK only, filter by gen_type_values name
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(tv.description, 'Unknown (' || (lm.notes::json->>'Type_TransactionEntryID') || ')'),
  ...
  INNER JOIN public.legacy_id_mappings tv
  ...
  WHERE lm.table_name = 'bookkeeping'
    ...
    AND tv.description ILIKE '%loan%'
    AND tv.description NOT ILIKE '%Loan FL Receivable (BS)%'
  ORDER BY (lm.notes::json->>'TransactionDate') ASC;
END;
$$;

-- Update outstanding calculation to match: BK only, filter by type values with "loan" in name
...
BEGIN
  ...
  SELECT
    ...
    e.name,
    e.last_name,
    a.total_credit AS total_payout,
    0::numeric AS total_loading,
    a.total_credit AS total_loan,
    a.total_debit AS total_repaid,
    0::numeric AS total_writeoff,
    a.total_credit - a.total_debit AS outstanding
  FROM aggregated a
  ...
  ORDER BY (a.total_credit - a.total_debit) DESC;
END;
$$;
