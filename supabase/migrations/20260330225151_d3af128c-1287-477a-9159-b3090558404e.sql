...
AS $$
  -- Balances from NON-LEGACY cashflow_transactions only (new/live transactions)
  SELECT
  ...
  WHERE ct.tenant_id = p_tenant_id
    ...
    AND ct.legacy_transaction_id IS NULL
    AND (p_up_to_date IS NULL OR ct.transaction_date <= p_up_to_date)
  GROUP BY ct.control_account_id

  UNION ALL

  -- ALL legacy CFT data from legacy_id_mappings (source of truth for historical data)
  SELECT
